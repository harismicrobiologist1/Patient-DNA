...
import { LoggedOutGateway } from "./components/LoggedOutGateway";
...
export default function App() {
  ...
  const [doctorAuthSessions, setDoctorAuthSessions] = useState<Record<string, DoctorAuthSession>>({
    // Initial patient is pre-authorized for seamless demo
    "DNA-8924-9012": {
  ...
  const currentRecord =
    ...
    Object.values(patientsDatabase)[0] ||
    INITIAL_PATIENTS_DATABASE["DNA-8924-9012"];
  ...
  const handleRegisterPatient = (newRecord: PatientFullRecord) => {
    ...
    const newLog: AuditLog = {
      ...
      ipAddress: "192.168.1.100 (Internal Secure System)",
  ...
  return (
    <div className="min-h-screen bg-slate-100/70 text-slate-900 font-sans antialiased selection:bg-blue-500 selection:text-white flex flex-col">
      ...
      {/* Main Navigation Sub-Bar with Lifetime Persistence Indicator */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-2">
            ...
            <div className="hidden lg:flex items-center space-x-2 pl-4 text-[11px] text-slate-500 font-medium shrink-0">
              <span className="flex items-center space-x-1 px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-200">
                <Database className="w-3 h-3 text-emerald-600" />
                <span>Lifetime Database Synced</span>
              </span>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* If logged out and not viewing Doctor or Hospital staff portals, show LoggedOutGateway */}
        {!isAuthenticated && activeTab !== "doctor-dash" && activeTab !== "hospital-dash" ? (
          <LoggedOutGateway
            patients={allPatientsList}
            onLoginSuccess={(dnaId) => {
            ...
            onSelectRoleTab={(tab) => setActiveTab(tab)}
          />
        ) : (
          <>
      ...
      {/* Add New Patient Registration Modal (Permanent Lifetime Storage) */}
      <AddPatientModal
      ...
      <footer className="bg-slate-900 border-t border-slate-800 text-slate-400 text-xs py-8 mt-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <p className="text-[11px] font-mono text-slate-500">
            Encrypted End-to-End • Persistent Lifetime Cloud Store • ISO 27001 Certified • HIPAA Compliant
          </p>
        </div>
      </footer>
    </div>
